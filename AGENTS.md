...
| Tipo | — |
| Cliente | — |
| Stack | Next.js 16.2.4 + React 19.2.4 + Supabase + Tailwind + Resend + MercadoPago + Zustand + Framer Motion + Lenis + Playwright + TypeScript + Vercel + E-commerce |
| Estado | desconocido |
| Último commit | — |
...
- `supabase-rls-funcion-policy-grant-execute` — **Bug silencioso — función en policy RLS necesita GRANT EXECUTE al rol que consulta**
  Cada vez que escribas o revises un `grant execute` sobre una función de Postgres en Supabase — en los dos…
...
- `recomprar-editar-dimension-nueva` — **\"Volver a pedir\" y \"editar\" quedan atrás cuando el carrito gana una segunda fuente de disponibilidad**
  Cuando a un carrito que ya andaba se le agrega una **segunda forma de comprar el mismo producto**: preventa…
...
- `i18n-editable-defaults-en-codigo` — **Textos editables por el cliente sin vaciar el diccionario del código**
  Cuando un cliente pide poder editar **todos** los textos de un sitio que ya tiene la copy hardcodeada en un…
...
- `webp-alfa-en-mails-y-uploaders` — **Iconos con fondo fantasma en un mail — WebP con alfa y padding transparente**
  Logos o iconos con transparencia que en un mail aparecen **con un fondo que no está en el archivo**, y/o se…
...
- `hash-reparto-celdas-colapsa` — **Repartir elementos en una grilla — id*k % total colapsa**
  Cuando haya que ubicar N elementos en casilleros de una grilla de forma determinista (la misma entidad…
...
- `nextjs-usesearchparams-prerender` — **Next.js — `useSearchParams` en ruta prerenderizada borra el subárbol del HTML del server**
  Cuando una página estática (`○` en la salida del build) llega al navegador **sin una parte de su contenido**…
...
- `capitalize-articulos-css` — **Capitulización correcta de frases con artículos en CSS**
  `capitalize` de CSS pone mayúscula en el primer carácter de CADA PALABRA — pero en frases con artículos…
...
- `migracion-correo-ferozo-google-workspace` — **Migración de correo Ferozo/DonWeb → Google Workspace**
  Migrar el correo de un cliente desde hosting compartido argentino (Ferozo / DonWeb / Dattatec) a Google…
  _citada por `emailjs-smtp-gmail-app-password`_
